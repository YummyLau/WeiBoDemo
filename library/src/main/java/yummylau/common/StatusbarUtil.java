 ...
 * 2. 支持fitsystemui下的状态栏着色，见{@link StatusbarUtil#setStatusbarColor}
 * 3. 支持头部imageview的沉浸，见{@link StatusbarUtil#setTranslucentForImageView}
...
public class StatusbarUtil {
        ...
        if (matchHandleStatusbarVersion()) {
            if (matchNearWhiteColor(color)) {
                if (matchLightStatusbarTextDevice()) {
                    ...
                    StatusbarUtil.lightStatusbarText(activity);
                }else {
     ...
     * 不会影响到导航栏
     * 同{@link StatusbarUtil#setTranslucentForImageView 基本一致 }
     *
     * @param activity
